...
- **`docs/ARTICLE_WRITING_PRINCIPLES.md`（記事執筆原則・基準文書）を必ず読む。** 本サイトの記事コンテンツを恒久的に導く執筆基準であり、すべての記事作成・見直しはこの原則（結論先出し＋自己完結型構成、検証データの見せ方、タイトルルール〔6章・6.1含む〕、行動経済学の許可/禁止リスト、体験の誠実性、恒久禁止事項）に従う。
...
- 想定クエリの**実SERP（Yahoo!JAPAN等）で競合タイトルの型**（ランキング／おすすめ◯選／実食レビュー・体験談）を確認し、本サイトの差別化語（「公式情報で検証」「◯社比較」「軸」「本文の本命テーゼ」）を決める（`ARTICLE_WRITING_PRINCIPLES.md` 6.1。参考: `docs/ARTICLE_TITLE_RESEARCH_AND_CANDIDATES_2026_09_04.md`・`docs/ARTICLE_TITLE_CANDIDATES_SERP_AUDIT_2026_09_04.md`）
...
6. CTA配置（記事内の公式サイトCTA回数・配置。既存記事の`aff_link(..., cls="btn-primary"/"btn-secondary")`パターンに合わせる。`btn-primary`は記事1本あたり2〜3箇所を上限とする＝4章10項）
...
9. 各H2セクション末尾に次見出しを予告する橋渡し文（同4章8項）、本文中盤（40〜60%地点）に内部リンク1つ（同4章9項）の配置を組み込む
...
- タイトルルール（6章・6.1）: 想定クエリ語を先頭に置き、答え・軸・対象を予告し、本文と一致する実数（社数等）を伴わせる。体験支配クエリ（「やめた理由」等）の追撃・「おすすめ/No.1」・網羅語・弱い語尾を使わない。タイトル・H1・冒頭段落・meta descriptionの文言は同期させる。鮮度表記（【◯年◯月最新】等）を使う場合は `LAST_VERIFIED_DATE` から動的算出する（**直書き禁止**。2026-09-02のUI監査で発見した鮮度ズレバグの再発防止）
...
- 見出しのギャップ提示は「実は」等の煽り語ではなく実データの数字を2つ並べる形にする（6章）。強調（太字）は1段落2箇所以内、感情語ではなく数値・サービス名にのみ使う（5章7項）
- AIらしさ防止（12章）: 本文に編集判断・検証プロセスの文を最低1つ置く（12.1）。読者案内の定型（〜しましょう/〜てほしい/〜どうぞ）は記事1本2回まで・橋渡し文の型を記事内で反復しない（12.2）。段落・文末に緩急をつけ、表へ流す情報と本文の役割を分担して本文での重複掲載を避ける（12.3・12.4）
...
2. **構成・一貫性パス**（章立て・整合性）
   - `ARTICLE_WRITING_PRINCIPLES.md` 11章のチェックリスト（21項目）を通す
   - 記事タイトル・見出しが実際の内容量・比較軸数と一致しているか（誇張がないか）
3. **文章パス**（表現・誠実性・AIらしさ）
   - 捏造した体験談・口コミ・緊急性演出・根拠のない断定表現がないか
   - 冗長な言い回し・同じ主張の繰り返しがないか
   - **AIらしさ（12章）**: 本文から書き手（編集部）の判断の文を取り除いても意味が通るか（12.1＝DB転記になっていないか）。読者案内の定型（〜しましょう/〜てほしい/〜どうぞ）が制限内か・橋渡し文の型が反復していないか（12.2）。段落・文末に緩急があるか・表と本文で同じ内容を重複掲載していないか（12.3・12.4）
4. **行動喚起パス**（読者の次の一歩）
...
- タイトルが6.1を満たすか（想定クエリ語が先頭／答え・軸の予告／本文と一致する実数／体験支配クエリや「おすすめ・No.1」を使っていない／全角36字前後／H1・冒頭段落と同期）
- 見出し階層（H1→H2→H3）が論理的か（各H2が異なる問いに対応し内容が重複していないか、H2の並び順が読者の意思決定プロセス〔気になる点の把握→比較→行動〕に沿っているか）、キーワード詰め込みになっていないか
